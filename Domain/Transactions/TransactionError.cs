...
{
    public static Error ExceedsMaximumLimit(decimal amount) => Error.Failure($"{nameof(Transaction)}.ExceedsMaximumLimit",$"The amount {amount} exceeds the maximum deposit limit.");

    public static Error Failed(string message) => Error.Failure($"{nameof(Transaction)}.Failed",$"Transaction failed: {message}");
